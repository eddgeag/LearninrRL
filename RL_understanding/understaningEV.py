...
def EV_MC_predictionQ_quicker(episodes,init,s_g,theta=0): ## policy evaluacion through Q
    Q_,pi_,S=init
    Q,pi=Q_.copy(),pi_.copy()
    Reward=[]
    for s in range(S):
...
time1=time.time()
Q_1=EV_MC_predictionQ_quicker(episodes,initq,s_g,theta=0);Q_1_=extractQmax(Q_1)
time2=time.time()
delta=time2-time1
print('end at',delta)
